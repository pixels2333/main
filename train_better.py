...
batch_size=32
# 创建训练数据集实例
train_dataset = MyData(image_folder='T_V_img_data/train_183_4758', excel_file='T_V_img_data/train_enlarge.xlsx',transform=transform)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
val_dataset = MyData(image_folder='T_V_img_data/val_46_1196', excel_file='T_V_img_data/val_enlarge.xlsx',transform=transform)
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True)
# 创建测试数据集实例
test_dataset = MyData(image_folder='Test', excel_file='test.xlsx', transform=transform)
...
optimizer = torch.optim.Adam(cnn.parameters(), lr=learning_rate, weight_decay=2.9527008730298823e-05)        #优化器调整为Adam算法并加入L1正则化
...
for i in range(3):
    ...
    nrmse = nrmse_score(predictions_tensor, targets_tensor, data_range=targets_tensor.max() - targets_tensor.min())

    # 作图
    plt.figure(figsize=(8, 6))
    plt.scatter(predictions, targets, color='blue', label='Actual data')
    plt.plot(predictions, predicted_values, color='red', linewidth=2, label=f'Fit: y={coefficients[0]:.2f}x+{intercept:.2f}')
